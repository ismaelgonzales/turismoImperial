...
import { IProductStripe, IPasajero } from './productStripe.interface';
...
export const ICartStore = signalStore(
    ...
    withMethods(
        (
            { products, busSeleccionado, pasajeros, ...store },
            toastSvc = inject(ToastrService),
            seleccionAsientosSvc = inject(SeleccionAsientosService),
        ) => {
            ...
            function updateProducts(pasajeros: any[], busSeleccionado: any) {
                ...
                const productos = pasajeros.map(
                    (pasajero: any, index: number) => {
                        const {
                            propietario,
                            asiento,
                            tipoDocumento,
                        ...
                        const product: IProductStripe = {
                            ...
                            title:
                                ...
                                `${propietario?.apellidos} ${propietario?.nombres}`, // Primera o segunda estructura
                            ...
                            documento:
                                documento || propietario?.numeroDocumento, // Número de documento
                            asiento: asiento,
                            bus: busId,
                            category: '',
                            subTotal: 0,
